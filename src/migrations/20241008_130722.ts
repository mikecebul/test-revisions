...
export async function up({ payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await payload.db.drizzle.run(sql`CREATE TABLE \`search_categories\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`relation_to\` text,
  ...
  await payload.db.drizzle.run(sql`CREATE INDEX \`search_categories_order_idx\` ON \`search_categories\` (\`_order\`);`)
  await payload.db.drizzle.run(sql`CREATE INDEX \`search_categories_parent_id_idx\` ON \`search_categories\` (\`_parent_id\`);`)
  await payload.db.drizzle.run(sql`CREATE INDEX \`search_slug_idx\` ON \`search\` (\`slug\`);`)
...
export async function down({ payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await payload.db.drizzle.run(sql`DROP TABLE \`search_categories\`;`)
